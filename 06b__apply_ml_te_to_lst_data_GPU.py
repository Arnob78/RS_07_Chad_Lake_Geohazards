...
def normalize_lst(lst_series):
    """Normalize LST from Celsius to 0-1 range"""
    # Assuming a reasonable range for the area, e.g., 0 to 60 Celsius
    return (lst_series - 0) / (60 - 0)
...
if __name__ == '__main__':
    ...
    # --- File Paths ---
    # Find the LST raster dynamically
    lst_dir = "/srv/AB_YU/RS_07_Chad_Lake_Geohazards/04__align_rasters"
    lst_path = find_raster_file(lst_dir, "LST_annual_aligned")
    print(f"Found LST file: {os.path.basename(lst_path)}")
    
    # Find the NDVI raster dynamically
    ndvi_dir = "/srv/AB_YU/RS_07_Chad_Lake_Geohazards/03__create_annual_mosaics"
    ndvi_path = find_raster_file(ndvi_dir, "ndvi_annual_composite")
    print(f"Found NDVI file: {os.path.basename(ndvi_path)}")
    
    # --- Raster Processing ---
    print("Opening raster files...")
    with rasterio.open(lst_path) as y_src, rasterio.open(ndvi_path) as x_src:
        # Basic validation
        ...
        if len(x_valid) > 0:
            ...
            y_normalized = normalize_lst(y_valid)
            ...
            csv_output_path = os.path.join(output_folder, "te_results_lst.csv")
        ...
        # Save output
        output_path = os.path.join(output_folder, "TE_LST_to_NDVI_GPU.tif")
